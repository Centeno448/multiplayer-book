...
void RoboCat::ProcessInput(float inDeltaTime, const InputState& inInputState) {
  ...
  // turning...
  float newRotation = GetRotation() + inInputState.GetDesiredHorizontalDelta() *
...
void RoboCat::AdjustVelocityByThrust(float inDeltaTime) {
  ...
  // to 0 velocity simulating acceleration makes the client prediction a bit
  // more complex
...
void RoboCat::ProcessCollisions() {
  ...
  // object is not efficient. it would be preferable to use a quad tree or some
  // other structure to minimize the number of collisions that need to be
  // tested.
    ...
    if (target != this && !target->DoesWantToDie()) {
      ...
      if (distSq < (collisionDist * collisionDist)) {
        ...
        if (target->HandleCollisionWithCat(this)) {
          ...
          // relative velocity...
          RoboCat* targetCat = dynamic_cast<RoboCat*>(target);
          if (targetCat) {
            relVel -= targetCat->mVelocity;
          }

          // got vel with dir between objects to figure out if they're moving
          // towards each other and if so, the magnitude of the impulse ( since
