...
bool MoveList::AddMoveIfNew(const Move& inMove) {
  // we might have already received this move in another packet ( since we're
  // sending the same move in multiple packets ) so make sure it's new...
...
void MoveList::RemovedProcessedMoves(
  ...
  while (!mMoves.empty() && mMoves.front().GetTimestamp() <=
