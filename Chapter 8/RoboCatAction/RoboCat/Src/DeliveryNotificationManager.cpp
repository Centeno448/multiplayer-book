...
DeliveryNotificationManager::DeliveryNotificationManager(
    bool inShouldSendAcks, bool inShouldProcessAcks)
...
void DeliveryNotificationManager::WriteAckData(
  ...
  // however, if we have more than 1, we'll make that 1 bit a 1 and then write 8
  // bits of how many packets we could do some statistical analysis to determine
  // if this is the best strategy but we'll use it for now
...
bool DeliveryNotificationManager::ProcessSequenceNumber(
  ...
  // is the sequence number less than our current expected sequence? silently
  // drop it. if this is due to wrapping around, we might fail to ack some
  // packets that we should ack, but they'll get resent, so it's not a big deal
  // note that we don't have to re-ack it because our system doesn't reuse
  // sequence numbers
    ...
    mNextExpectedSequenceNumber = sequenceNumber + 1;
    // we should nack the missing packets..this will happen automatically inside
    // AddPendingAck because we're adding an unconsequitive ack and then we can
    // ack this and process it
    if (mShouldSendAcks) {
      AddPendingAck(sequenceNumber);
    }
    return true;
...
// anything in flight before the range will be considered nackd by the other
// side immediately
void DeliveryNotificationManager::ProcessAcks(
  ...
  if (hasAcks) {
    ...
    // for each InfilghtPacket with a sequence number less than the start,
    ...
    while (nextAckdSequenceNumber < onePastAckdSequenceNumber &&
      ...
      // if the packet has a lower sequence number, we didn't get an ack for it,
      ...
      if (nextInFlightPacketSequenceNumber < nextAckdSequenceNumber) {
        ...
        // want to find it when checking for state
...
void DeliveryNotificationManager::ProcessTimedOutPackets() {
  ...
  while (!mInFlightPackets.empty()) {
    ...
    // was this packet dispatched before the current time minus the timeout
    // duration?
    if (nextInFlightPacket.GetTimeDispatched() < timeoutTime) {
      ...
      // it wasn't, and packets are all in order by time here, so we know we
      // don't have to check farther
      break;
    }
  }
}
...
void DeliveryNotificationManager::AddPendingAck(
  ...
  // if you don't have a range yet, or you can't correctly extend the final
  // range with the sequence number, start a new range
