...
ScoreBoardManager::Entry::Entry(uint32_t inPlayerId,
...
void ScoreBoardManager::AddEntry(uint32_t inPlayerId,
                                 const std::string &inPlayerName) {
  // if this player id exists already, remove it first- it would be crazy to
  // have two of the same id
  RemoveEntry(inPlayerId);

  mEntries.emplace_back(inPlayerId, inPlayerName,
                        mDefaultColors[inPlayerId % mDefaultColors.size()]);
}
