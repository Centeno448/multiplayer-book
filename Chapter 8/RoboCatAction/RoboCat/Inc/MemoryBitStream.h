...
inline float ConvertFromFixed(uint32_t inNumber, float inMin,
                              float inPrecision) {
...
class OutputMemoryBitStream {
 public:
  ...
  &inData, inBitCount ); } void Write( int16_t inData, uint32_t inBitCount = 16
  )	{ WriteBits( &inData, inBitCount ); }

  void Write( uint8_t inData, uint32_t inBitCount = 8 )	{ WriteBits( &inData,
  inBitCount ); }
  */
