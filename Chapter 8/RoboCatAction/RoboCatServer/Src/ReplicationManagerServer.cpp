    ...
    if (replicationCommand.HasDirtyState()) {
      ...
      switch (action) {
        ...
        case RA_Destroy:
          ...
          writtenState =
              WriteDestroyAction(inOutputStream, networkId, dirtyState);
          break;
      }
...
uint32_t ReplicationManagerServer::WriteUpdateAction(
  ...
  // if we can't find the gameObject on the other side, we won't be able to read
  // the written data ( since we won't know which class wrote it ) so we need to
...
uint32_t ReplicationManagerServer::WriteDestroyAction(
    OutputMemoryBitStream& inOutputStream, int inNetworkId,
    uint32_t inDirtyState) {
  (void)inOutputStream;
  (void)inNetworkId;
