...
void NetworkManagerServer::ProcessPacket(InputMemoryBitStream& inInputStream,
  ...
  auto it = mAddressToClientMap.find(inFromAddress);
  if (it == mAddressToClientMap.end()) {
    // didn't find one? it's a new cilent..is the a HELO? if so, create a client
    // proxy...
...
void NetworkManagerServer::ProcessPacket(ClientProxyPtr inClientProxy,
  ...
  switch (packetType) {
    case kHelloCC:
      ...
      // the one we expect from this address, otherwise something weird is going
      // on...
...
void NetworkManagerServer::HandlePacketFromNewClient(
  ...
  if (packetType == kHelloCC) {
    ...
    ClientProxyPtr newClientProxy =
        std::make_shared<ClientProxy>(inFromAddress, name, mNewPlayerId++);
