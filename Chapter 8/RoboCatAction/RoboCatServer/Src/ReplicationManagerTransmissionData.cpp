...
void ReplicationManagerTransmissionData::AddTransmission(
  ...
  /*
  //it would be silly if we already had a transmission for this network id in
  here... for( const auto& transmission: mTransmissions )
  {
...
void ReplicationManagerTransmissionData::HandleDeliverySuccess(
    DeliveryNotificationManager* inDeliveryNotificationManager) const {
  // run through the transmissions, if any are Destroyed then we can remove this
  // network id from the map
  for (const ReplicationTransmission& rt : mTransmissions) {
    switch (rt.GetAction()) {
      case RA_Create:
...
void ReplicationManagerTransmissionData::HandleCreateDeliveryFailure(
  ...
  // does the object still exist? it might be dead, in which case we don't
  // resend a create
...
void ReplicationManagerTransmissionData::HandleCreateDeliverySuccess(
    int inNetworkId) const {
  // we've received an ack for the create, so we can start sending as only an
  // update
