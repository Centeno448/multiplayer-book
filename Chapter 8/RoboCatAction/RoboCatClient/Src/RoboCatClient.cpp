...
void RoboCatClient::Update() {
  ...
  if (GetPlayerId() == NetworkManagerClient::sInstance->GetPlayerId()) {
    ...
    // in theory, only do this if we want to sample input this frame / if
    // there's a new move ( since we have to keep in sync with server )
    if (pendingMove)  // is it time to sample a new move...
    {
      ...
      // LOG( "Client Move Time: %3.4f deltaTime: %3.4f left rot at %3.4f",
      // latestMove.GetTimestamp(), deltaTime, GetRotation() );
    }
  } else {
...
void RoboCatClient::Read(InputMemoryBitStream& inInputStream) {
  ...
  if (GetPlayerId() == NetworkManagerClient::sInstance->GetPlayerId()) {
    ...
    // will this smooth us out too? it'll interpolate us just 10% of the way
    // there...
    if ((readState & ECRS_PlayerId) == 0) {
      InterpolateClientSidePrediction(oldRotation, oldLocation, oldVelocity,
                                      true);
...
void RoboCatClient::DoClientSidePredictionAfterReplicationForLocalCat(
  ...
  if ((inReadState & ECRS_Pose) != 0) {
    // simulate pose only if we received new pose- might have just gotten
    // thrustDir in which case we don't need to replay moves because we haven't
    // warped backwards

    // all processed moves have been removed, so all that are left are
    // unprocessed moves so we must apply them...
...
void RoboCatClient::InterpolateClientSidePrediction(
    float inOldRotation, const Vector3& inOldLocation,
  ...
  if (!RoboMath::Is2DVectorEqual(inOldLocation, GetLocation())) {
    ...
    if (durationOutOfSync < roundTripTime) {
      SetLocation(
          Lerp(inOldLocation, GetLocation(),
...
void RoboCatClient::DoClientSidePredictionAfterReplicationForRemoteCat(
  ...
  if ((inReadState & ECRS_Pose) != 0) {
    ...
    // let's break into framerate sized chunks though so that we don't run
    // through walls and do crazy things...
