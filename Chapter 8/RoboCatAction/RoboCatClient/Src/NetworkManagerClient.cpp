...
void NetworkManagerClient::HandleGameObjectState(
    InputMemoryBitStream& inInputStream) {
...
void NetworkManagerClient::HandleScoreBoardState(
    InputMemoryBitStream& inInputStream) {
  ScoreBoardManager::sInstance->Read(inInputStream);
}

void NetworkManagerClient::DestroyGameObjectsInMap(
    const IntToGameObjectMap& inObjectsToDestroy) {
...
void NetworkManagerClient::SendInputPacket() {
  ...
  if (moveList.HasMoves()) {
    ...
    // eventually write the 3 latest moves so they have three chances to get
    // through...
