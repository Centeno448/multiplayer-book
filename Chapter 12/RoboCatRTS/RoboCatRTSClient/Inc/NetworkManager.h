...
class NetworkManager {
 ...
 private:
  void ProcessPacketsLobby(InputMemoryBitStream& inInputStream,
                           uint64_t inFromPlayer);
  void ProcessPacketsReady(InputMemoryBitStream& inInputStream,
                           uint64_t inFromPlayer);
  void HandleReadyPacket(InputMemoryBitStream& inInputStream,
                         uint64_t inFromPlayer);
  void SendReadyPacketsToPeers();
  void HandleStartPacket(InputMemoryBitStream& inInputStream,
                         uint64_t inFromPlayer);
  void ProcessPacketsPlaying(InputMemoryBitStream& inInputStream,
                             uint64_t inFromPlayer);
  void HandleTurnPacket(InputMemoryBitStream& inInputStream,
                        uint64_t inFromPlayer);
  void ProcessPacketsDelay(InputMemoryBitStream& inInputStream,
                           uint64_t inFromPlayer);
  void TryStartGame();
 ...
 private:
  ...
  typedef std::map<uint64_t, std::string> Int64ToStrMap;
  typedef std::map<uint64_t, TurnData> Int64ToTurnDataMap;
  ...
  std::vector<Int64ToTurnDataMap> mTurnData;
