...
class Command {
 public:
  ...
  Command() : mCommandType(CM_INVALID), mNetworkId(0), mPlayerId(0) {}

  // given a buffer, will construct the appropriate command subclass
  static std::shared_ptr<Command> StaticReadAndCreate(
...
class AttackCommand : public Command {
 public:
  AttackCommand() : mTargetNetId(0) { mCommandType = CM_ATTACK; }

  static std::shared_ptr<AttackCommand> StaticCreate(uint32_t inMyNetId,
...
class MoveCommand : public Command {
 public:
  MoveCommand() { mCommandType = CM_MOVE; }

  static std::shared_ptr<MoveCommand> StaticCreate(uint32_t inNetworkId,
