...
class GamerServices {
 public:
  ...
  void GetLobbyPlayerMap(uint64_t inLobbyId,
                         std::map<uint64_t, std::string>& outPlayerMap);
  void SetLobbyReady(uint64_t inLobbyId);
  void LeaveLobby(uint64_t inLobbyId);

  // peer-to-peer networking
  bool SendP2PReliable(const OutputMemoryBitStream& inOutputStream,
                       uint64_t inToPlayer);
  bool IsP2PPacketAvailable(uint32_t& outPacketSize);
  uint32_t ReadP2PPacket(void* inToReceive, uint32_t inMaxLength,
...
#undef STAT
  ...
  enum class StatType { INT, FLOAT, AVGRATE };

  int GetStatInt(Stat inStat);
  float GetStatFloat(Stat inStat);
  float GetStatAvgRate(Stat inStat);

  // for int/float stats, we can just add to the stat and write to the server
  // immediately
  ...
  // then write the average rate for the interval to the server, resetting the
  // local cache
