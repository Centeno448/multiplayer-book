...
class GameObject {
 public:
  ...
  virtual bool HandleCollisionWithCat(GameObject* inObj) {
    (void)inObj;
    return true;
  }
  ...
  void SetPlayerId(uint64_t inPlayerId) { mPlayerId = inPlayerId; }
  uint64_t GetPlayerId() const { return mPlayerId; }

  // no default implementation because we don't know what's relevant for each
  // object necessarily
