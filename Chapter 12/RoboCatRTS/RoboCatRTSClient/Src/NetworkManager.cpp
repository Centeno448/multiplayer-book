...
NetworkManager::NetworkManager()
    : mBytesSentThisFrame(0),
      ...
      mTimeToStart(-1.0f),
      // we always start on turn -2 b/c we need 2 frames before we can actually
      // play
      mTurnNumber(-2),
      mSubTurnNumber(0) {
...
void NetworkManager::UpdateDelay() {
  ...
  if (mState == NMS_Delay) {
    ...
    std::unordered_set<uint64_t> playerSet;
...
void NetworkManager::TryAdvanceTurn() {
  ...
  if (mTurnData[mTurnNumber + 1].size() == mPlayerCount) {
    ...
    mState = NMS_Delay;
    LOG("Going into delay state, don't have all the info for turn %d",
        mTurnNumber + 1);
  }
}

void NetworkManager::ProcessPacket(InputMemoryBitStream& inInputStream,
                                   uint64_t inFromPlayer) {
  switch (mState) {
    case NMS_Lobby:
...
void NetworkManager::ProcessPacketsLobby(InputMemoryBitStream& inInputStream,
                                         uint64_t inFromPlayer) {
...
void NetworkManager::ProcessPacketsReady(InputMemoryBitStream& inInputStream,
                                         uint64_t inFromPlayer) {
...
void NetworkManager::HandleReadyPacket(InputMemoryBitStream& inInputStream,
                                       uint64_t inFromPlayer) {
  // if this is my first ready packet, I need to let everyone else know I'm
  // ready
  if (mReadyCount == 0) {
...
void NetworkManager::HandleStartPacket(InputMemoryBitStream& inInputStream,
                                       uint64_t inFromPlayer) {
  ...
  if (inFromPlayer == mMasterPeerId) {
    ...
    // for now, assume that we're one frame off, but ideally we would RTT to
    // adjust the time to start, based on latency/jitter
    mState = NMS_Starting;
    mTimeToStart = kStartDelay - Timing::sInstance.GetDeltaTime();
  }
}

void NetworkManager::ProcessPacketsPlaying(InputMemoryBitStream& inInputStream,
                                           uint64_t inFromPlayer) {
  uint32_t packetType;
  inInputStream.Read(packetType);
...
void NetworkManager::HandleTurnPacket(InputMemoryBitStream& inInputStream,
                                      uint64_t inFromPlayer) {
  int turnNum;
  uint64_t playerId;
  ...
  if (playerId != inFromPlayer) {
    LOG("We received turn data for a different player Id...stop trying to "
        "cheat!");
    return;
  }
...
void NetworkManager::ProcessPacketsDelay(InputMemoryBitStream& inInputStream,
                                         uint64_t inFromPlayer) {
...
void NetworkManager::HandleConnectionReset(uint64_t inFromPlayer) {
  ...
  if (mPlayerNameMap.find(inFromPlayer) != mPlayerNameMap.end()) {
    ...
    // if we were in delay, then let's see if we can continue now that this
    // player DC'd?
    if (mState == NMS_Delay) {
      TryAdvanceTurn();
    }
...
void NetworkManager::ReadIncomingPacketsIntoQueue() {
  ...
  InputMemoryBitStream inputStream(packetMem,
                                   static_cast<uint32_t>(packetSize * 8));
  uint64_t fromPlayer;

  // keep reading until we don't have anything to read ( or we hit a max number
  // that we'll process per frame )
  ...
  while (GamerServices::sInstance->IsP2PPacketAvailable(incomingSize) &&
    ...
    if (incomingSize <= packetSize) {
      uint32_t readByteCount = GamerServices::sInstance->ReadP2PPacket(
          packetMem, static_cast<uint32_t>(packetSize), fromPlayer);
      if (readByteCount > 0) {
        inputStream.ResetToCapacity(readByteCount);
        ++receivedPackedCount;
        totalReadByteCount += readByteCount;

        // shove the packet into the queue and we'll handle it as soon as we
        // should... we'll pretend it wasn't received until simulated latency
        // from now this doesn't sim jitter, for that we would need to.....
        float simulatedReceivedTime =
            Timing::sInstance.GetTimef() + mSimulatedLatency;
  ...
  if (totalReadByteCount > 0) {
    mBytesReceivedPerSecond.UpdatePerSecond(
...
void NetworkManager::EnterPlayingState() {
  ...
  std::array<Vector3, 4> spawnLocs = {
      Vector3(-halfWidth + halfWidth / 5, -halfHeight + halfHeight / 5,
              2.35f),  // UP-LEFT
      Vector3(-halfWidth + halfWidth / 5, halfHeight - halfHeight / 4,
              -5.49f),  // DOWN-LEFT
      Vector3(halfWidth - halfWidth / 5, halfHeight - halfHeight / 4,
              -0.78f),  // DOWN-RIGHT
      Vector3(halfWidth - halfWidth / 5, -halfHeight + halfHeight / 4,
  ...
  std::shuffle(indices.begin(), indices.end(),
               RandGen::sInstance->GetGeneratorRef());
  ...
  for (auto& iter : mPlayerNameMap) {
    ...
    if (spawnVec.mX > 0.0f) {
      SpawnCat(iter.first,
               Vector3(spawnVec.mX - kCatOffset, spawnVec.mY, spawnVec.mZ));
    } else {
      SpawnCat(iter.first,
               Vector3(spawnVec.mX + kCatOffset, spawnVec.mY, spawnVec.mZ));
    }
    ...
    if (spawnVec.mY > 0.0f) {
      SpawnCat(iter.first,
               Vector3(spawnVec.mX, spawnVec.mY - kCatOffset, spawnVec.mZ));
    } else {
      SpawnCat(iter.first,
               Vector3(spawnVec.mX, spawnVec.mY + kCatOffset, spawnVec.mZ));
    }
  ...
  for (int i = 0; i < GamerServices::MAX_ACHIEVEMENT; ++i) {
    ...
    switch (ach) {
      case GamerServices::ACH_WIN_ONE_GAME:
        if (GamerServices::sInstance->GetStatInt(GamerServices::Stat_NumWins) >
            0) {
...
bool NetworkManager::CheckSync(Int64ToTurnDataMap& inTurnMap) {
  ...
  while (iter != inTurnMap.end()) {
    if (expectedRand != iter->second.GetRandomValue()) {
      LOG("Random is out of sync for player %llu on turn %d",
          iter->second.GetPlayerId(), mTurnNumber);
      return false;
    }

    if (expectedCRC != iter->second.GetCRC()) {
      LOG("CRC is out of sync for player %llu on turn %d",
          iter->second.GetPlayerId(), mTurnNumber);
      return false;
    }
    ++iter;
...
void NetworkManager::UpdateBytesSentLastFrame() {
  if (mBytesSentThisFrame > 0) {
    mBytesSentPerSecond.UpdatePerSecond(
...
NetworkManager::ReceivedPacket::ReceivedPacket(
    float inReceivedTime, InputMemoryBitStream& ioInputMemoryBitStream,
    uint64_t inFromPlayer)
...
uint32_t NetworkManager::ComputeGlobalCRC() {
  ...
  crc = static_cast<uint32_t>(
      crc32(crc, reinterpret_cast<const Bytef*>(crcStream.GetBufferPtr()),
