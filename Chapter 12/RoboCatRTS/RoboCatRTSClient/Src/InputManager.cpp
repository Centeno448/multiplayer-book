    ...
    switch (inKeyCode) {
      ...
      case SDLK_F10:
        // DEBUG: upload score to leaderboard
        {
          ...
          int killCount =
              GamerServices::sInstance->GetStatInt(GamerServices::Stat_NumWins);
          GamerServices::sInstance->UploadLeaderboardScoreAsync(
              GamerServices::LB_KillCount, killCount);
          break;
        }
      case SDLK_F11:
        // DEBUG: download leaderboard rankings
        {
          ...
          GamerServices::sInstance->DownloadLeaderboardAsync(
              GamerServices::LB_KillCount);
          break;
        }
      default:
        break;
...
void InputManager::HandleMouseClick(int32_t inX, int32_t inY, uint8_t button) {
  ...
  switch (button) {
    case SDL_BUTTON_LEFT:
      mSelectedNetId =
          World::sInstance->TrySelectGameObject(Vector3(worldX, worldY, 0.0f));
      break;
