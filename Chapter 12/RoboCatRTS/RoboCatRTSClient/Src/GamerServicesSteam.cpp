...
struct AchieveData {
  ...
  AchieveData(const char* inName) : Name(inName) {}
};
...
struct LeaderboardData {
  ...
  LeaderboardData(const char* inName, ELeaderboardSortMethod inMethod,
      ...
      : Name(inName), SortMethod(inMethod), DisplayType(inDisplay) {}
};
...
struct GamerServices::Impl {
  ...
  STEAM_CALLBACK(Impl, OnP2PSessionRequest, P2PSessionRequest_t,
                 mSessionRequestCallback);
  // Callback if the P2P connection fails
  STEAM_CALLBACK(Impl, OnP2PSessionFail, P2PSessionConnectFail_t,
                 mSessionFailCallback);
...
GamerServices::Impl::Impl()
    :  // use an x-macro to construct every element of the stat/achievement
       // arrays
      mStatArray({
...
#undef ACH
      ...
      mLeaderArray({
#define BOARD(a, b, c)                             \
  LeaderboardData(#a, k_ELeaderboardSortMethod##b, \
                  k_ELeaderboardDisplayType##c),
...
void GamerServices::Impl::OnLobbyCreateCallback(LobbyCreated_t* inCallback,
                                                bool inIOFailure) {
...
void GamerServices::Impl::OnLobbyEnteredCallback(LobbyEnter_t* inCallback,
                                                 bool inIOFailure) {
  if (inIOFailure) {
    return;
  }
...
void GamerServices::Impl::OnLeaderUploadCallback(
    LeaderboardScoreUploaded_t* inCallback, bool inIOFailure) {
  ...
  if (!inIOFailure) {
    ...
    for (int i = 0; i < inCallback->m_cEntryCount; ++i) {
      SteamUserStats()->GetDownloadedLeaderboardEntry(
          inCallback->m_hSteamLeaderboardEntries, i, &entry, nullptr, 0);
...
void GamerServices::Impl::OnP2PSessionFail(
    P2PSessionConnectFail_t* inCallback) {
...
std::string GamerServices::GetLocalPlayerName() {
  return std::string(SteamFriends()->GetPersonaName());
}

std::string GamerServices::GetRemotePlayerName(uint64_t inPlayerId) {
...
void GamerServices::LobbySearchAsync() {
  ...
  SteamMatchmaking()->AddRequestLobbyListStringFilter("game", kGameName,
...
void GamerServices::GetLobbyPlayerMap(
    uint64_t inLobbyId, std::map<uint64_t, std::string>& outPlayerMap) {
...
bool GamerServices::SendP2PReliable(const OutputMemoryBitStream& inOutputStream,
                                    uint64_t inToPlayer) {
  return SteamNetworking()->SendP2PPacket(
      inToPlayer, inOutputStream.GetBufferPtr(), inOutputStream.GetByteLength(),
      k_EP2PSendReliable);
}
...
uint32_t GamerServices::ReadP2PPacket(void* inToReceive, uint32_t inMaxLength,
  ...
  uint32_t packetSize;
  CSteamID fromId;
  SteamNetworking()->ReadP2PPacket(inToReceive, inMaxLength, &packetSize,
                                   &fromId);
...
void GamerServices::RetrieveStatsAsync() {
  // SteamUserStats()->RequestCurrentStats(); deprecated
}
...
void GamerServices::UpdateAvgRateLocal(Stat inStat, float inIncValue,
                                       float inIncSeconds) {
  if (!mImpl->mAreStatsReady) {
    LOG("Stats ERROR: Stats not ready yet");
    return;
...
void GamerServices::WriteAvgRate(Stat inStat) {
  ...
  SteamUserStats()->UpdateAvgRateStat(stat.Name, stat.AvgRateStat.SessionValue,
...
void GamerServices::UploadLeaderboardScoreAsync(Leaderboard inLead,
                                                int inScore) {
  ...
  SteamAPICall_t call = SteamUserStats()->UploadLeaderboardScore(
      mImpl->mLeaderArray[inLead].Handle,
      k_ELeaderboardUploadScoreMethodKeepBest, inScore, NULL, 0);

  mImpl->mLeaderUploadResult.Set(call, mImpl.get(),
                                 &Impl::OnLeaderUploadCallback);
}
...
void GamerServices::DownloadLeaderboardAsync(Leaderboard inLead) {
  ...
  mImpl->mLeaderDownloadResult.Set(call, mImpl.get(),
                                   &Impl::OnLeaderDownloadCallback);
}
...
bool GamerServices::StaticInit() {
  ...
  if (!retVal) {
    LOG("Failed to initialize Steam. Make sure you are running a Steam "
        "client.");
  } else {
