...
bool WindowManager::StaticInit() {
  int screenW = static_cast<int>(ceilf(kWorldZoomFactor * kWorldWidth));
  int screenH = static_cast<int>(ceilf(kWorldZoomFactor * kWorldHeight));
  SDL_Window* wnd = SDL_CreateWindow("Robo Cat RTS", 100, 100, screenW, screenH,
