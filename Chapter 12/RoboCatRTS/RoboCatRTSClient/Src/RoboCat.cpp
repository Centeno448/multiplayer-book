...
void RoboCat::TakeDamage(GameObjectPtr inFromCat, int inDmgAmount) {
  mHealth -= inDmgAmount;
  if (mHealth == 0) {
    SetDoesWantToDie(true);

    // update stats as necessary
    if (inFromCat) {
      uint64_t playerId = NetworkManager::sInstance->GetMyPlayerId();
      if (mPlayerId == playerId) {
        // did our cat die?
        GamerServices::sInstance->AddToStat(GamerServices::Stat_NumLosses, 1);
      } else if (inFromCat->GetPlayerId() == playerId) {
        // did we kill a cat?
        GamerServices::sInstance->AddToStat(GamerServices::Stat_NumWins, 1);
      }
    }
  }
}
