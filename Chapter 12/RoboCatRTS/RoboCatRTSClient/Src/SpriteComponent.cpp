...
void SpriteComponent::Draw(const SDL_Rect& inViewTransform) {
  if (mTexture) {
    ...
    // Blit the texture
    SDL_RenderCopyEx(render, mTexture->GetData(), nullptr, &dstRect,
                     RoboMath::ToDegrees(mGameObject->GetRotation()), nullptr,
                     SDL_FLIP_NONE);

    if (mGameObject->GetNetworkId() ==
        InputManager::sInstance->GetSelectedNetId()) {
