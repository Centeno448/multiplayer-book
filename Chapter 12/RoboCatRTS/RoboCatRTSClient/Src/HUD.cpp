...
HUD::HUD()
    : mScoreBoardOrigin(30.f, 60.f, 0.0f),
      ...
      mHealth(0) {
  TTF_Init();
  mFont = TTF_OpenFont(CARLITO_FONT, 36);
  if (mFont == nullptr) {
...
void HUD::RenderTurnNumber() {
  std::string str = StringUtils::Sprintf(
      "Turn %d:%d", NetworkManager::sInstance->GetTurnNumber(),
      NetworkManager::sInstance->GetSubTurnNumber());
  RenderText(str, mBandwidthOrigin, Colors::Blue);
}

void HUD::StaticInit() { sInstance.reset(new HUD()); }
...
void HUD::RenderHealth() {
  if (mHealth > 0) {
    std::string healthString = StringUtils::Sprintf("Health %d", mHealth);
...
void HUD::RenderCountdown() {
  NetworkManager::NetworkManagerState state =
      NetworkManager::sInstance->GetState();
  ...
  switch (state) {
    case NetworkManager::NMS_Starting: {
      ...
      if (timeToStart > 0.0f) {
        std::string timeStr =
            StringUtils::Sprintf("STARTING IN %.2f", timeToStart);
        RenderText(timeStr, mTimeOrigin, Colors::Red);
      }
    } break;
    case NetworkManager::NMS_Lobby: {
      std::string str = StringUtils::Sprintf(
          "%d/4 players in lobby", NetworkManager::sInstance->GetPlayerCount());
      RenderText(str, mTimeOrigin, Colors::Blue);
    } break;
    case NetworkManager::NMS_Ready: {
      std::string str = StringUtils::Sprintf(
          "%d/%d players ready", NetworkManager::sInstance->GetReadyCount(),
          NetworkManager::sInstance->GetPlayerCount());
      RenderText(str, mTimeOrigin, Colors::Blue);
    } break;
...
void HUD::RenderBandWidth() {
  std::string bandwidth = StringUtils::Sprintf(
      "In %d  Bps, Out %d Bps",
      static_cast<int>(
          NetworkManager::sInstance->GetBytesReceivedPerSecond().GetValue()),
      static_cast<int>(
          NetworkManager::sInstance->GetBytesSentPerSecond().GetValue()));
  RenderText(bandwidth, mBandwidthOrigin, Colors::White);
}

void HUD::RenderScoreBoard() {
  const std::vector<ScoreBoardManager::Entry>& entries =
...
void HUD::RenderText(const std::string& inStr, const Vector3& origin,
  ...
  SDL_Texture* texture = SDL_CreateTextureFromSurface(
      GraphicsDriver::sInstance->GetRenderer(), surface);
  ...
  // Draw the texture
  SDL_RenderCopy(GraphicsDriver::sInstance->GetRenderer(), texture, nullptr,
                 &dstRect);
